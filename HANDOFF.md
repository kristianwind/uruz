...
| `await push(...)` and assuming the server has it | The offline queue awaited only the IndexedDB write and sent with `void flush()`. Anything asking the server about what it just wrote lost the race — that is why only set 1 ever got a PR. |
| Hiding a form field instead of clearing the value | The weight control was hidden for bodyweight exercises; the save path still sent it. A crunch got 41 kg nobody could see. Hide the field *and* the value, or show it. |
