 ...
 */
export function workoutExercisesFromSets(sets: SetLog[]): ReusedExercise[] {
  const order: string[] = [];
  ...
  return order.slice(0, MAX_EXERCISES).map((exerciseId, index) => {
    ...
    return {
      ...
      targetRepsMin: reps.length > 0 ? Math.min(...reps) : null,
      targetRepsMax: reps.length > 0 ? Math.max(...reps) : null,
      ...
      targetSeconds: reps.length === 0 && seconds.length > 0 ? Math.max(...seconds) : null,
